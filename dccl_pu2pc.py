...
from Datasets.data_pre import load_data_pavia, get_source_data, get_target_data
...
data_path_s = '/tmp/DA_HSIC/PublicDataset/Pavia/paviaU.mat'  # Please change the path to the path where you store your dataset.
label_path_s = '/tmp/DA_HSIC/PublicDataset/Pavia/paviaU_gt_7.mat'
data_path_t = '/tmp/DA_HSIC/PublicDataset/Pavia/pavia.mat'
label_path_t = '/tmp/DA_HSIC/PublicDataset/Pavia/pavia_gt_7.mat'

set_seed(876)

source_data, source_label = load_data_pavia(data_path_s, label_path_s)  # (610, 340, 102) (610, 340,)
target_data, target_label = load_data_pavia(data_path_t, label_path_t)  # (1096, 715, 102) (1096, 715,)
...
loader_s_no_drop = DataLoader(train_dataset_s, batch_size=BATCH_SIZE, shuffle=True, drop_last=False)
loader_t_no_drop = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=True, drop_last=False)
...
def train(total_epochs):
    ...
    cont_tt = SupConLoss(temperature=0.5).cuda()
    ...
    for ep in range(total_epochs):
        ...
        for batch_idx, data in enumerate(zip(train_loader_s, train_loader_t)):
            ...
            if total_indices.shape == torch.Size([0]):
            ...
            elif total_indices.shape == torch.Size([1]):
                ...
                cdc_loss += cont_ta(features_s, selected_feature_t, label_s, total_pseudo_label)
                idc_loss = cont_ss(features_s, label_s)
                idc_loss += 0.3 * cont_tt(selected_feature_t, total_pseudo_label)
            ...
            all_loss = cls_loss + 0.4 * cdc_loss + 0.4 * idc_loss
            all_loss.backward()
